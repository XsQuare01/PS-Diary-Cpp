...
### 해설
<p>
 자신의 왼쪽에 있는 탑 중 가장 가까운 탑을 찾는 문제입니다. 과정은 다음과 같습니다.

 1. 탑을 왼쪽부터 입력받고, 자신보다 큰 탑이 나올 때까지 스택을 pop 합니다.  
    pop을 해도 문제없는 이유는, 자기보다 높이가 낮으면서 왼쪽에 있는 탑들은 어짜피 자신에 가려저 보이지 않게 되기 때문입니다.
 2. 자신보다 큰 높이의 탑이 나온다면 스택에 push 합니다.
 3. 1-2 과정을 반복합니다.

문제에서 탑의 번호를 요구하였기에 스택의 자료형을 pair<int, int>로 설정하였습니다.  
또한, 자신보다 큰 탑이 없는 경우 예외 처리를 위해 스택의 맨 바닥에 {INF, 0}을 넣어두었습니다.
</p>
