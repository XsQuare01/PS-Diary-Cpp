...
### 해설
<p>
 최종 상태는 다음과 같은 상황만이 존재합니다.

 1. 모든 보드판이 채워지고 O 가 한 줄이 되지 않았을 때
 2. 모든 보드판이 채워지지 않고 (X 개수) = (O 개수) + 1 이며, X 한 줄만 완성이 되었을 때
 3. 모든 보드판이 채워지지 않고 (X 개수) = (O 개수) 이며, O 한 줄만 완성이 되었을 때 O 한 줄만 완성이 되었을 때

 따라서, 모든 라인에 대해 각각의 경우만 확인해주면 됩니다.
